...
readonly BUGCLERK_JAR=${BUGCLERK_JAR:-"${HOME}/.m2/repository/org/jboss/jbossset/bugclerk-dist/1.0.6.Final-SNAPSHOT/bugclerk-dist-1.0.6.Final-SNAPSHOT-shaded.jar"}
readonly JSOUP_JAR=${JSOUP_JAR:-'/home/rpelisse/.m2/repository/org/jsoup/jsoup/1.12.1/jsoup-1.12.1.jar'}
...
scala  -Daphrodite.config=${APHRODITE_CONFIG}  -classpath "${CLASSPATH}" "${SCRIPT}" ${@}  2>&1 | \
    ...
    -e '/INFO: Aphrodite Initialisation Complete/d' -e '/INFO: Initiating Aphrodite .../d'
